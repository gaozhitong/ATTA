...
class Distortion(object):
    def __init__(self):
        self.color_jitter = trans.ColorJitter(0.8, 0.8, 0.8, 0.2)
        self.gaussian_blur = trans.RandomApply([
            trans.GaussianBlur((3, 3), (1.0, 2.0))
        ], p=0.5)

    def __call__(self, tensor, mask, *inputs):
...
class GaussianBlur(object):
    def __init__(self):
        self.gaussian_blur =  trans.GaussianBlur((9, 9), (0.1, 5.0))
    def __call__(self, tensor, mask, *inputs):
        tensor = self.gaussian_blur(tensor)
        return tensor, mask, tuple(i for i in inputs)
...
class Resize(object):
    def __init__(self, size, *inputs, **kwargs):
        self.t1 = trans.Resize(size)
        self.t2 = trans.Resize(size, trans.InterpolationMode.NEAREST)

    def __call__(self, tensor, mask, *inputs, **kwargs):

        return self.t1(tensor), self.t2(mask.unsqueeze(0)).squeeze(0), tuple(i for i in inputs)
...
class ResizeImg(object):
    ...
    def __call__(self, tensor, mask, *inputs, **kwargs):
        ...
        return self.t1(tensor), mask, tuple(i for i in inputs)
